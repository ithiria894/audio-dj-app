# 需求 — the founder 原話 (source of truth)

> 呢份係 the founder 喺同 ChatGPT 對話入面**自己講嘅需求原話**，係成個 project 嘅真相源。
...
5. **關鍵定義（the founder 自己講清楚咗）**：「總之手機播乜就 display 啲乜」「我隻/我個耳機聽乜，人哋就聽乜」→ 即係 **system audio streaming**，唔係「大家各自同步播同一首歌」。
...
Folder 用 working name `audio-dj-app`；正式名遲啲用 `/name-product` 或 the founder 決定。
