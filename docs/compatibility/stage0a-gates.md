...
- **Gate 3.1** service-owned session, **zero tracks**: move Room/LiveKit lifecycle from Activity into `AudioCaptureService`; Activity only does consent + start/stop; `LOCAL_PROOF` vs `LIVEKIT_PUBLISH` mutually exclusive; connect from service, publish 0 tracks, background the Activity, browser still sees pixel-host, mode NORMAL, stop→leaves.
