...
> Spec-first。範圍決定基於 `02-feasibility-report.md` + `03-competitive-report.md` + Nicole 拍板。
...
**證咩:**(a) Android `AudioPlaybackCapture` 喺 Nicole 部機 capture 到播緊嘅聲;(b) 邊個音源 app 真係出到聲 vs 靜音(尤其 **Spotify**);(c) 音訊-only session 鎖屏後**唔停**(feasibility 話豁免,實機驗)。
**點證:**
...
- 通知常駐 + 狀態 log。
**通過準則:** Nicole 播本地音樂,錶郁 + WAV 有聲;鎖屏後錶仍郁。逐個 app 播記低邊個出聲。
